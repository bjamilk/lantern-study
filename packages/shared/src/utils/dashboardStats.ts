/**
 * Shared dashboard-stats types. Single source of truth for web and mobile
 * so both surfaces compute identical dashboard metrics.
 * Import via subpath (`@lantern/shared/utils/dashboardStats`) — intentionally
 * not re-exported from the utils barrel to avoid `Badge` name collisions.
 */
import type { StudyActivityDay } from '../types';
import { XP_LEVELS, getXPLevel } from './levels';
