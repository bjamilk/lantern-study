...
**Resolved — the round trip now works end to end.** A second account (David, iOS)
opened `lanternstudy://invite/invite-…` generated by the first account and **joined**:
the app navigated into the group and the header went from "1 members" to "2 members".
So C2 → C3 → C4 are verified all the way through, non-member included.

Getting there needed three native fixes, all committed:
`330f7a7` patches `@expo/plist` for xmldom 0.9 (unbreaks `expo prebuild` *and*
`expo run:ios`), and `6c1ff90` adds `plugins/withFmtCxx17.js` plus a slider podspec
patch so the iOS project compiles at all. Note `expo run:ios` is still unusable on this
machine — `devicectl` misreports under Xcode 26 so Expo treats the booted simulator as a
physical device and demands signing certs; build with `xcodebuild` against the simulator
destination instead.
