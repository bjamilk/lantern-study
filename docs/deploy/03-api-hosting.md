...
| `SUPADATA_API_KEY` | **Required for YouTube import** — from [supadata.ai](https://supadata.ai) (free tier: 100 transcript credits/month) |

`render.yaml` sets `REDIS_ENABLED=true`. Without `REDIS_URL`, the API will **fail to start** in production.

**YouTube note import:** The API fetches transcripts via Supadata (not direct YouTube scraping). Sign up at [supadata.ai](https://supadata.ai), copy your API key, and set `SUPADATA_API_KEY` on Render. Transcripts are cached in Redis for 7 days per video ID (re-importing the same URL does not use another credit). Without this key, `POST /api/v1/notes/youtube-import` returns **503** in production.
