...
const CreateGroupScreen: React.FC<CreateGroupScreenProps> = ({ currentUser, allUsers, onCreateGroup, onBack }) => {
      ...
      <div key="step-select" className="flex-1 flex flex-col bg-slate-100 dark:bg-slate-900">
            ...
            <div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm p-4">
              ...
              <div className="flex flex-wrap gap-2">
                {selectedUsers.map(user => (
                  ...
                  >
                    <img src={getAvatarUrl(user)} alt={user.name} className="w-5 h-5 rounded-full mr-2" onError={(e) => { e.currentTarget.src = "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' fill='%239ca3af' viewBox='0 0 24 24'%3E%3Cpath d='M12 12c2.7 0 4.8-2.1 4.8-4.8S14.7 2.4 12 2.4 7.2 4.5 7.2 7.2 9.3 12 12 12zm0 2.4c-3.2 0-9.6 1.6-9.6 4.8v2.4h19.2v-2.4c0-3.2-6.4-4.8-9.6-4.8z'/%3E%3C/svg%3E"; }} />
        ...
        {selectedUserIds.length > 0 && (
          <div className="p-4 bg-white dark:bg-slate-800 border-t dark:border-slate-700">
            <button
              ...
              className="w-full py-3 bg-blue-600 text-white rounded-lg font-semibold flex items-center justify-center hover:bg-blue-700"
            >
        ...
        {selectedUserIds.length === 0 && (
          <div className="p-4 bg-white dark:bg-slate-800 border-t dark:border-slate-700">
            <button
              onClick={() => setStep('group_details')}
              className="w-full py-3 bg-gray-600 text-white rounded-lg font-semibold flex items-center justify-center hover:bg-gray-700"
            >
    ...
    <div key="step-details" className="flex-1 flex flex-col bg-slate-100 dark:bg-slate-900">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
            <div className="flex flex-col items-center space-y-3">
                 <button onClick={() => avatarFileRef.current?.click()} className="relative w-24 h-24 rounded-full bg-gray-200 dark:bg-slate-700 flex items-center justify-center group">
                    {avatarPreview ? (
                        <img src={avatarPreview} alt="Group avatar preview" className="w-full h-full object-cover rounded-full" onError={(e) => { e.currentTarget.src = "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' fill='%239ca3af' viewBox='0 0 24 24'%3E%3Cpath d='M12 12c2.7 0 4.8-2.1 4.8-4.8S14.7 2.4 12 2.4 7.2 4.5 7.2 7.2 9.3 12 12 12zm0 2.4c-3.2 0-9.6 1.6-9.6 4.8v2.4h19.2v-2.4c0-3.2-6.4-4.8-9.6-4.8z'/%3E%3C/svg%3E"; }} />
                    ) : (
            ...
            <div className="max-w-sm mx-auto p-4 bg-white dark:bg-slate-800 rounded-lg shadow-sm">
                <h3 className="font-semibold text-gray-800 dark:text-gray-200 flex items-center mb-2"><LockClosedIcon className="w-5 h-5 mr-2 text-gray-500 dark:text-gray-400"/>Member Permissions</h3>
