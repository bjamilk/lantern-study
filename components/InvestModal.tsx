...
const SavingsGoalModal: React.FC<SavingsGoalModalProps> = ({ isOpen, onClose, currentUserId }) => {
  const { savingsGoals, addSavingsGoal, contributeTo, removeSavingsGoal } = useBudgetStore();
  ...
  const handleCreateGoal = () => {
    ...
    addSavingsGoal(goal);
    setName(''); setTargetAmount(''); setIcon('🎯'); setDeadline('');
    setMode('list');
  };

  const handleContribute = () => {
    if (!contributeGoalId || !contributeAmount || contributeAmount <= 0) return;
    contributeTo(contributeGoalId, Number(contributeAmount));
    setContributeAmount('');
    setMode('list');
  };
        ...
        <div className="p-5 overflow-y-auto flex-1">
                    ...
                    return (
                      <div key={goal.id} className="bg-slate-50 dark:bg-slate-700/50 rounded-xl p-4">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <button onClick={() => removeSavingsGoal(goal.id)} className="text-slate-300 hover:text-red-500 text-xs">✕</button>
                        </div>
          ...
          {mode === 'create' && (
            <div className="space-y-4">
              ...
              <div className="flex gap-3 pt-1">
                ...
                <button onClick={handleCreateGoal} disabled={!name.trim() || !targetAmount} className="flex-1 px-4 py-2.5 text-sm font-semibold text-white bg-amber-500 hover:bg-amber-600 rounded-xl shadow-sm disabled:opacity-40">Create Goal</button>
              </div>
            </div>
          )}
          ...
          {mode === 'contribute' && contributeGoal && (
            <div className="space-y-4">
              ...
              <div className="flex gap-3 pt-1">
                <button onClick={() => setMode('list')} className="flex-1 px-4 py-2.5 text-sm font-medium text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-700 rounded-xl hover:bg-slate-200 dark:hover:bg-slate-600">Back</button>
                <button onClick={handleContribute} disabled={!contributeAmount || contributeAmount <= 0} className="flex-1 px-4 py-2.5 text-sm font-semibold text-white bg-amber-500 hover:bg-amber-600 rounded-xl shadow-sm disabled:opacity-40">Add Funds</button>
              </div>
            </div>
          )}
        </div>
