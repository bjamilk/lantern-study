...
const AddIncomeModal: React.FC<AddIncomeModalProps> = ({ isOpen, onClose, onSubmit }) => {
  ...
  // closeAfter=false keeps the sheet open for logging several entries in a row.
  const save = (closeAfter: boolean) => {
    if (amount === '' || amount <= 0) {
      useToastStore.getState().showToast('Please enter a valid positive amount.', 'error');
      return;
    }
    const selected = STUDENT_INCOME_CATEGORIES.find(c => c.id === category);
    onSubmit({
    ...
    if (closeAfter) {
      ...
      useToastStore.getState().showToast('Income added ✓', 'success');
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      ariaLabelledBy="add-income-modal-title"
      maxWidthClass="max-w-md"
      panelClassName="!p-0 overflow-hidden"
    >
      <div className="bg-gradient-to-r from-lantern-success to-emerald-600 px-5 py-4 flex justify-between items-center">
        <h2 id="add-income-modal-title" className="text-lg font-bold text-white">Add Income</h2>
        <button
          type="button"
          onClick={onClose}
      ...
      <form onSubmit={handleSubmit} className="p-5 space-y-4 bg-lantern-surface">
        <div>
          <label htmlFor="income-amount" className="block text-sm font-medium text-lantern-text mb-1">Amount (₦)</label>
          <div className="relative">
            ...
            <input
              ...
              min="0.01"
              step="0.01"
              className="w-full min-h-[44px] p-2.5 pl-8 border border-lantern-border rounded-xl bg-lantern-surface text-lantern-text text-lg font-semibold focus:ring-2 focus:ring-lantern-success focus:border-transparent"
              placeholder="0.00"
            />
          </div>
        </div>
        <div>
          <p className="block text-sm font-medium text-lantern-text mb-1">Source</p>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2" role="group" aria-label="Income source">
        ...
        <div>
          <label htmlFor="income-description" className="block text-sm font-medium text-lantern-text mb-1">Description <span className="text-lantern-text-muted font-normal">(optional)</span></label>
          <input
            type="text"
            id="income-description"
            value={description}
            onChange={(e) => setDescription(e.target.value)}
            className="w-full min-h-[44px] p-2.5 border border-lantern-border rounded-xl bg-lantern-surface text-lantern-text focus:ring-2 focus:ring-lantern-success focus:border-transparent"
            placeholder="e.g. Allowance from Dad"
          />
        </div>
        <div>
          <label htmlFor="income-date" className="block text-sm font-medium text-lantern-text mb-1">Date</label>
          <input
            type="date"
            id="income-date"
            value={date}
        ...
        <div className="space-y-2 pt-1">
          <button
            type="submit"
            className="w-full min-h-[44px] px-4 py-2.5 text-sm font-semibold text-white bg-lantern-success hover:opacity-90 rounded-xl shadow-sm"
          >
            Add income
          </button>
          <div className="flex gap-3">
            ...
            <button
              type="button"
              onClick={() => save(false)}
              className="flex-1 min-h-[44px] px-4 py-2.5 text-sm font-medium text-lantern-success bg-lantern-success/10 rounded-xl hover:bg-lantern-success/20"
            >
              Add another
            </button>
          </div>
