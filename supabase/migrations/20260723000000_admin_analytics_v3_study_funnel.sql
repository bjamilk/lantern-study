-- admin_analytics v3: adds consent-gated study funnel from product_events
-- (test/flashcard sessions, notes, AI tool usage). No schema changes.
  ...
  SELECT jsonb_build_object(
    ...
    'studyFunnel', (
      SELECT jsonb_build_object(
        'testsStarted', (
          SELECT COUNT(*)::int FROM product_events
          WHERE event = 'test_started' AND created_at >= v_start::timestamptz
        ),
        'testsCompleted', (
          SELECT COUNT(*)::int FROM product_events
          WHERE event = 'test_completed' AND created_at >= v_start::timestamptz
        ),
        'testsCompletedWeb', (
          SELECT COUNT(*)::int FROM product_events
          WHERE event = 'test_completed' AND surface = 'web'
            AND created_at >= v_start::timestamptz
        ),
        'testsCompletedMobile', (
          SELECT COUNT(*)::int FROM product_events
          WHERE event = 'test_completed' AND surface = 'mobile'
            AND created_at >= v_start::timestamptz
        ),
        'flashcardSessionsStarted', (
          SELECT COUNT(*)::int FROM product_events
          WHERE event = 'flashcard_review_started' AND created_at >= v_start::timestamptz
        ),
        'flashcardSessionsCompleted', (
          SELECT COUNT(*)::int FROM product_events
          WHERE event = 'flashcard_review_completed' AND created_at >= v_start::timestamptz
        ),
        'notesCreated', (
          SELECT COUNT(*)::int FROM product_events
          WHERE event = 'note_created' AND created_at >= v_start::timestamptz
        ),
        'aiToolUses', (
          SELECT COUNT(*)::int FROM product_events
          WHERE event = 'ai_tool_used' AND created_at >= v_start::timestamptz
        ),
        'aiToolsByType', (
          SELECT COALESCE(jsonb_object_agg(tool, cnt), '{}'::jsonb)
          FROM (
            SELECT COALESCE(props->>'tool', 'unknown') AS tool, COUNT(*)::int AS cnt
            FROM product_events
            WHERE event = 'ai_tool_used' AND created_at >= v_start::timestamptz
            GROUP BY 1
            ORDER BY 2 DESC
...
COMMENT ON FUNCTION public.admin_analytics(INTEGER) IS
  'Platform admin dashboard aggregates including marketplace GMV, retention, study funnel, and product_events (service_role only).';
