-- Private profileVisibility must remain searchable (basic identity).
-- Discoverability opt-out stays on discoverableForInvites.
-- Mutual blocks still hide users from each other's search results.
-- Full profile RLS (profile_visible_to) is unchanged — private still hides the full profile.
...
COMMENT ON FUNCTION public.search_users(text, uuid, uuid, integer) IS
  'Search users by name or @username. Private profiles are searchable; opt out via discoverableForInvites. Blocks hide mutual results. Prefers prefix matches; contains fallback for queries >= 3 chars.';
