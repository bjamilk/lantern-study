-- Search privacy M2: separate discoverableForInvites from profileVisibility.
-- Revoke direct authenticated RPC access; API (service_role) only.
...
COMMENT ON FUNCTION public.search_users(text, uuid, uuid, integer) IS
  'Username search for group/deck invites. Excludes private profiles and users who opted out of invite discovery.';
