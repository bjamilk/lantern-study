-- Reserve listings when an order opens (offer accept / buy now); restore on cancel.
...
ALTER TABLE public.marketplace_listings
  ...
  CHECK (status IN ('active', 'inactive', 'sold', 'reserved'));
...
-- Release escrow: accept reserved listings as in-flight sales.
...
-- Backfill: any listing with an open order should be reserved.
