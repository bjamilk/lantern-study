-- Restore name + username matching in search_users (regression fix).
-- Keeps discoverableForInvites and profileVisibility privacy filters.
...
COMMENT ON FUNCTION public.search_users(text, uuid, uuid, integer) IS
  'Search users by username or name for invites. Excludes private profiles and users who opted out of invite discovery.';
