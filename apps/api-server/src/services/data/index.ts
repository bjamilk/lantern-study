...
function createMarketplaceApi(
  ...
  return {
    ...
    // Moved out of routes/marketplace/* (lane R2, PR 3).
    createSavedSearch: bindDb(client, marketplaceData.createSavedSearch),
    listSavedSearches: bindDb(client, marketplaceData.listSavedSearches),
    deleteSavedSearch: bindDb(client, marketplaceData.deleteSavedSearch),
    updateSavedSearch: bindDb(client, marketplaceData.updateSavedSearch),
    getSellerProfileCard: bindDb(client, marketplaceData.getSellerProfileCard),
    listSellerListings: bindDb(client, marketplaceData.listSellerListings),
    listReviewsForListings: bindDb(client, marketplaceData.listReviewsForListings),
    countFavoritesForListings: bindDb(client, marketplaceData.countFavoritesForListings),
    countInquiriesForListings: bindDb(client, marketplaceData.countInquiriesForListings),
    getQuestionBankMeta: bindDb(client, marketplaceData.getQuestionBankMeta),
    getStudyPackMeta: bindDb(client, marketplaceData.getStudyPackMeta),
    getDigitalEntitlement: bindDb(client, marketplaceData.getDigitalEntitlement),
    expireOfferIfPending: bindDb(client, marketplaceData.expireOfferIfPending),
    listOrdersForOffers: bindDb(client, marketplaceData.listOrdersForOffers),
    insertOffer: bindDb(client, marketplaceData.insertOffer),
    findPendingOfferForBuyer: bindDb(client, marketplaceData.findPendingOfferForBuyer),
    listOffersForParty: bindDb(client, marketplaceData.listOffersForParty),
    getOfferWithListing: bindDb(client, marketplaceData.getOfferWithListing),
    getOfferById: bindDb(client, marketplaceData.getOfferById),
    setOfferStatusIfPending: bindDb(client, marketplaceData.setOfferStatusIfPending),
    listOffersForListing: bindDb(client, marketplaceData.listOffersForListing),
    counterOfferRpc: bindDb(client, marketplaceData.counterOfferRpc),
    updateOrderFieldsAsParty: bindDb(client, marketplaceData.updateOrderFieldsAsParty),
...
function createTestsApi(
  ...
  return {
    ...
    listSetPreAssessments: bindDb(client, testsData.listSetPreAssessments),
