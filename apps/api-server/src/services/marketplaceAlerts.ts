...
const INTERVAL_MS = parseInt(
  process.env.MARKETPLACE_ALERTS_INTERVAL_MS || "900000",
  10,
);
...
async function batchCreateNotifications(
  ...
  if (error) {
    logger.warn("Batch notification insert failed", {
      error: error.message,
      count: rows.length,
    });
    return 0;
...
export async function processSavedSearchAlerts(
  ...
  if (error) {
    logger.error("Failed to load saved searches for alerts", {
      error: error.message,
    });
    return 0;
  }
  ...
  for (const search of searches || []) {
    ...
    if (listErr) {
      logger.warn("Saved search listing query failed", {
        searchId: search.id,
        listErr,
      });
      continue;
    ...
    const matches = (listings || [])
      .filter((l) => listingMatchesFilters(l, filters))
      .slice(0, 5);
    ...
    for (const n of existingNotifications || []) {
      ...
      if (!data) continue;
      if (
        data.saved_search_id === search.id &&
...
export async function processReviewReminders(
  ...
  const { data: orders, error } = await db
    ...
    .select(
      "id, buyer_id, listing_id, completed_at, listing:marketplace_listings(title)",
    )
    .eq("status", "completed")
  ...
  for (const order of orders) {
    const listingRaw = order.listing as
      | { title?: string }
      | { title?: string }[]
      | null;
...
const ABANDONED_ORDER_MINUTES = parseInt(
  process.env.MARKETPLACE_ABANDONED_ORDER_MINUTES || "45",
  10,
);
const STALE_OFFER_HOURS = parseInt(
  process.env.MARKETPLACE_STALE_OFFER_HOURS || "24",
  10,
);
const OFFER_EXPIRY_WARNING_HOURS = parseInt(
  process.env.MARKETPLACE_OFFER_EXPIRY_WARNING_HOURS || "6",
  10,
);
...
export async function processAbandonedCheckoutReminders(
  ...
  const { data: orders, error } = await db
    .from("marketplace_orders")
    .select(
      "id, buyer_id, listing_id, created_at, listing:marketplace_listings(title)",
    )
...
export async function processStaleOfferReminders(
  ...
  const expirySoon = new Date(
    Date.now() + OFFER_EXPIRY_WARNING_HOURS * 60 * 60 * 1000,
  ).toISOString();
  const now = new Date().toISOString();
  ...
  const { data: staleForSeller, error: sellerErr } = await db
    ...
    .select(
      "id, seller_id, buyer_id, amount, listing_id, listing:marketplace_listings(title)",
    )
    .eq("status", "pending")
  ...
  const { data: expiringForBuyer, error: buyerErr } = await db
    .from("marketplace_offers")
    .select(
      "id, buyer_id, amount, listing_id, expires_at, listing:marketplace_listings(title)",
    )
    .eq("status", "pending")
    .gt("expires_at", now)
...
export function startMarketplaceAlertJobs(
  ...
  if (process.env.ENABLE_MARKETPLACE_JOBS !== "true") {
    logger.info(
      "Marketplace alert jobs disabled (set ENABLE_MARKETPLACE_JOBS=true to enable)",
    );
    return;
  }
