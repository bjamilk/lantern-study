/**
 * Mobile AI Service
 * Calls the backend AI API endpoints.
 * Mirrors the web ai.ts service but uses mobile API patterns.
 */
...
/**
 * Fetch current AI usage for a user
 */
export async function fetchAIUsage(userId: string): Promise<AIUsageInfo> {
  try {
    const headers = await getAuthHeaders();
    const res = await fetch(
      `${API_BASE_URL}/api/v1/ai/usage?userId=${encodeURIComponent(userId)}`,
      { headers }
    );
...
async function aiRequest<T>(endpoint: string, body: Record<string, any>): Promise<T> {
  ...
  const timeoutId = setTimeout(() => controller.abort(), 30000); // 30s for AI
