 ...
 * current tab now draws the SAME glyph solid — `AppIcon`'s `filled` — so the
 * shape itself changes, with the bold label as the third signal.
 *
 * Not every glyph survives being filled. lucide draws several as a container
 * plus interior detail, and filling paints the detail out: `person-circle`
 * (CircleUser) becomes a plain disc with the face erased. Any icon on this
 * bar has to read solid, which is why Me uses `person` (User) — head and
 * shoulders, unmistakable at 24px in either state.
 *
