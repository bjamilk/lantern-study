    ...
    <SafeAreaView className="flex-1 bg-lantern-background" edges={['top']}>
      ...
      <ScrollView contentContainerClassName="px-4 pb-8 gap-3">
        <Card className="p-4 gap-3">
          <TextInput value={title} onChangeText={setTitle} placeholder="Expense title" placeholderTextColor="#94a3b8" className="border border-lantern-border rounded-xl px-4 py-3 text-lantern-text dark:text-white bg-lantern-surface" />
          ...
          <TextInput value={participant} onChangeText={setParticipant} placeholder="Participant name" placeholderTextColor="#94a3b8" className="border border-lantern-border rounded-xl px-4 py-3 text-lantern-text dark:text-white bg-lantern-surface" />
        ...
        {expenseSplits.map((split: ExpenseSplit) => (
          <Card key={split.id} className="p-4">
            <View className="flex-row justify-between">
              <Text className="font-semibold text-lantern-text dark:text-white">{split.title}</Text>
              <Text className="text-lantern-primary font-medium">{formatCurrency(split.totalAmount)}</Text>
            </View>
            {split.participants.map((p, idx) => (
              <Pressable key={idx} onPress={() => void toggleSplitParticipantPaid(split.id, idx)} className="flex-row items-center gap-2 mt-2">
                ...
                <Text className={`text-sm ${p.paid ? 'text-lantern-text-tertiary line-through' : 'text-lantern-text'}`}>
