  ...
  return (
    <SafeAreaView className="flex-1 bg-slate-50 dark:bg-slate-900" edges={['top']}>
      <View className="px-4 pt-2 pb-3 flex-row items-center">
        ...
        <Text className="text-xl font-bold text-slate-900 dark:text-slate-100">Create Listing</Text>
      </View>

      <ScrollView className="flex-1 px-4" contentContainerStyle={{ paddingBottom: 32 }}>
        <Text className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Photos</Text>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} className="mb-4">
          ...
          {images.length < MAX_IMAGES ? (
            <Pressable
              onPress={pickImages}
              className="w-20 h-20 rounded-xl border border-dashed border-slate-300 items-center justify-center"
            >
              {uploading ? (
        ...
        <Text className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Title *</Text>
        <TextInput
          value={title}
          onChangeText={setTitle}
          placeholder="What are you selling?"
          placeholderTextColor="#94a3b8"
          className="p-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 mb-4"
        />

        <Text className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Category *</Text>
        <Pressable
          onPress={() => setShowCategories(v => !v)}
          className="p-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 mb-2 flex-row items-center justify-between"
        >
          <Text className="text-slate-900 dark:text-slate-100">{selectedCategory?.name ?? 'Select category'}</Text>
            ...
            {ALL_CATEGORIES.map(cat => (
              <Pressable
                ...
                className={`px-3 py-1.5 rounded-full border ${
                  category === cat.id ? 'bg-indigo-600 border-indigo-600' : 'border-slate-200 dark:border-slate-600'
                }`}
              >
                <Text className={`text-xs font-medium ${category === cat.id ? 'text-white' : 'text-slate-600'}`}>
                  {cat.name}
                </Text>
              </Pressable>
            ))}
        ...
        <Text className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Price (₦)</Text>
        <TextInput
          value={price}
        ...
        <Text className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Quantity in stock (optional)</Text>
        <TextInput
          value={quantity}
        ...
        <Text className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Promotion (optional)</Text>
        <TextInput
          value={salePrice}
          ...
          placeholderTextColor="#94a3b8"
          keyboardType="numeric"
          className="p-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 mb-2"
        />
          ...
          {(['none', '24h', '7d'] as const).map(preset => (
            <Pressable
              ...
              className={`px-3 py-1.5 rounded-lg border ${
                saleEndsPreset === preset ? 'bg-indigo-600 border-indigo-600' : 'border-slate-200 dark:border-slate-600'
              }`}
            >
              <Text className={`text-xs font-medium ${saleEndsPreset === preset ? 'text-white' : 'text-slate-600'}`}>
        ...
        <Text className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Location</Text>
        <TextInput
          value={location}
        ...
        <TextInput
          ...
          className="min-h-[120px] p-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 mb-6"
        />
