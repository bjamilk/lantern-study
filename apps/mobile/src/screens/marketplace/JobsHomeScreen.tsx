  ...
  const load = useCallback(async (requestedPage = 1) => {
    ...
    try {
      const response = await fetchJobPostings({
        ...
        employmentType:
          filter === 'full_time' || filter === 'part_time' || filter === 'internship'
            ? filter
            : undefined,
      ...
      setJobs((current) =>
        requestedPage === 1 ? response.data || [] : [...current, ...(response.data || [])]
      );
    ...
    <View className="flex-1 bg-lantern-background">
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ paddingBottom: 32 }}>
        <View className="px-4 pb-4">
          <Text className="text-2xl font-bold text-lantern-text">Find your next opportunity</Text>
        ...
        <View className="mx-4 rounded-2xl border border-lantern-border bg-lantern-surface p-3">
          ...
          >
            <Text className="text-sm font-semibold text-white">Search jobs</Text>
          </Pressable>
        </View>
            ...
            >
              <Text
                className={`text-sm font-medium ${
                  filter === option.id ? 'text-white' : 'text-lantern-text-secondary'
                }`}
              >
                {option.label}
              </Text>
        ...
        <View className="mx-4 mb-4 flex-row gap-2">
          ...
          >
            <Text className="text-xs font-semibold text-lantern-primary">My applications</Text>
          </Pressable>
          <Pressable
            onPress={() => navigation.navigate('MyJobPostings')}
            className="flex-1 items-center rounded-xl border border-lantern-border bg-lantern-surface px-2 py-3"
          >
            <Text className="text-xs font-semibold text-lantern-primary">My job posts</Text>
          </Pressable>
          <Pressable
            onPress={() => navigation.navigate('JobEmployer')}
            className="flex-1 items-center rounded-xl border border-lantern-border bg-lantern-surface px-2 py-3"
          >
            <Text className="text-xs font-semibold text-lantern-primary">Employer hub</Text>
          </Pressable>
        </View>
        ...
        <View className="px-4">
          ...
          {loading ? <ActivityIndicator color="#0f766e" className="my-8" /> : null}
          {error ? (
                ...
                >
                  <Card
                    className={`mb-3 border ${
                      job.isSponsored ? 'border-amber-300' : 'border-lantern-border'
                    }`}
                  >
                    <View className="flex-row gap-3">
                      ...
                      <View className="min-w-0 flex-1">
                        ...
                        <Text className="mt-1 text-sm text-lantern-text-secondary" numberOfLines={1}>
                          {employer}
                    ...
                    <Text className="mt-3 text-sm leading-5 text-lantern-text-secondary" numberOfLines={2}>
                      {job.description}
                    </Text>
          ...
          {!loading && !error && jobs.length === 0 ? (
            <View className="rounded-2xl border border-dashed border-lantern-border bg-lantern-surface p-8">
              ...
              >
                <Text className="text-sm font-semibold text-lantern-primary">Clear filters</Text>
              </Pressable>
            </View>
          ) : null}
          ...
          {!loading && jobs.length < total ? (
            ...
            >
              {loadingMore ? (
                ...
                <Text className="text-sm font-semibold text-lantern-primary">Load more jobs</Text>
              )}
            </Pressable>
          ) : null}
