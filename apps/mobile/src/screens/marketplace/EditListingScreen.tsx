    ...
    <SafeAreaView className="flex-1 bg-slate-50 dark:bg-slate-900" edges={['top']}>
      <KeyboardAvoidingView className="flex-1" behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <View className="px-4 pt-2 pb-3 flex-row items-center">
          ...
          <Text className="text-xl font-bold text-slate-900 dark:text-slate-100">Edit Listing</Text>
        </View>
        ...
        <ScrollView className="flex-1 px-4" contentContainerStyle={{ paddingBottom: 32 }}>
          ...
          >
            <Text className="text-slate-900 dark:text-slate-100">{selectedCategory?.name ?? category}</Text>
          ...
          {showCategories ? (
            <View className="flex-row flex-wrap gap-2 mb-4">
              {ALL_CATEGORIES.map(cat => (
                ...
                >
                  <Text className={`text-xs ${category === cat.id ? 'text-white' : 'text-slate-600'}`}>
                    {cat.name}
                  </Text>
                </Pressable>
              ))}
